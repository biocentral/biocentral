...
from biocentral_server.protein_analysis.analysis_functions import (
...
class AnalysisFunctionsTests(unittest.TestCase):
    ...
    def _get_random_nt_sequences(n: int = 20) -> Dict[str, str]:
        ...
        seqs = sorted(
            [
                "".join(random.choices("ATGC", k=random.randint(12, 16)))
                for _ in range(n)
            ]
        )
    ...
    def test_calculate_levenshtein_distances(self):
        ...
        self.assertEqual(
            len(sequence_ratios), 20, "Did not get expected amount of ratios!"
        )
        outer_keys = []
        for seq_out, result_dict in sequence_ratios.items():
            ...
            self.assertTrue(
                all([seq in inner_keys for seq in seqs.keys()]),
                "Missing sequence key (inner) in ratio dict!",
            )

        self.assertTrue(
            all([seq in outer_keys for seq in seqs.keys()]),
            "Missing sequence key (outer) in ratio dict!",
        )
