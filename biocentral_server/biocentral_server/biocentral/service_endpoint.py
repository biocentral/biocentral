...
@biocentral_service_route.route(
    "/biocentral_service/hashes/<hash_id>/<file_type>", methods=["GET"]
)
...
def transfer_file():
    ...
    if file_manager.check_file_exists(
        ...
        return jsonify(
            {
                "error": "Hash already exists at server, "
...
@biocentral_service_route.route(
    "/biocentral_service/task_status/<task_id>", methods=["GET"]
)
...
# Endpoint to check task status
@biocentral_service_route.route(
    "/biocentral_service/task_status_resumed/<task_id>", methods=["GET"]
)
