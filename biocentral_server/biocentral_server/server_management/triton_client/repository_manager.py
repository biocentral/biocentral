...
class TritonRepositoryManager:
    ...
    _instance: Optional["TritonRepositoryManager"] = None
    ...
    @classmethod
    def get_instance(cls) -> "TritonRepositoryManager":
    ...
    def get_repository(
        self, config: Optional[TritonClientConfig] = None
        ...
        if self._repository is None:
            with self._connection_lock:
                if self._repository is None:
                    ...
                    logger.info(
                        f"Creating Triton repository for worker (PID: {threading.get_ident()})"
                    )
                    self._repository = create_triton_repository(config)
                    self._repository.connect()
                    logger.info(
                        f"Triton repository connected at {config.triton_grpc_url}"
                    )
...
def get_shared_repository(
    ...
    Returns:
        TritonInferenceRepository instance
    """
    manager = TritonRepositoryManager.get_instance()
...
def cleanup_repositories() -> None:
    ...
    """
    TritonRepositoryManager.cleanup_all()
