...
class TrainingDTOObserver(BiotrainerOutputObserver):
    ...
    def update(self, data: OutputData) -> None:
        ...
        if data.derived_values:
            dto = TaskDTO.running().add_update(update={"derived_values": data.derived_values})
            self.update_dto_callback(dto)

        if data.training_iteration:
            split_name, metrics = data.training_iteration
            dto = TaskDTO.running().add_update(
                update={"training_iteration": {"split_name": split_name, "metrics": metrics}})
            self.update_dto_callback(dto)

        if data.test_results:
            dto = TaskDTO.running().add_update(update={"test_results": data.test_results})
            self.update_dto_callback(dto)

        if data.split_specific_values:
            dto = TaskDTO.running().add_update(update={"split_specific_values": data.split_specific_values})
            self.update_dto_callback(dto)
