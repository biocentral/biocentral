...
class ProteinPreprocessor:
    ...
    def preprocess_sequences(self, sequences: List[str]) -> List[str]:
        ...
        sequences_cleaned = [
            re.sub(
                rf"[{self.characters_to_replace}]", self.replacement_character, sequence
            )
        ...
        if self.uses_whitespaces:
            sequences_spaced = [
                " ".join(list(sequence)) for sequence in sequences_cleaned
            ]
        else:
...
class TritonPythonModel:
    def initialize(self, args):
        ...
        self.tokenizer = T5Tokenizer.from_pretrained(
            model_path, do_lower_case=False, legacy=False
        )
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            if sequences_array.ndim == 1:
                ...
                sequences = [
                    seq.decode("utf-8") if isinstance(seq, bytes) else seq
