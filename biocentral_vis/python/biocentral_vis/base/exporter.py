...
class CrossPlatformMetadataExporter:
    ...
    def export(self) -> Dict[str, Any]:
        ...
        output = {"svg": self.svg_string, "metadata": self.metadata}
    ...
    def _extract_metadata_from_altair(chart: alt.Chart) -> Dict[str, Any]:
        ...
        x_label = CrossPlatformMetadataExporter._extract_axis_label(
            encoding.get("x", {})
        )
        y_label = CrossPlatformMetadataExporter._extract_axis_label(
            encoding.get("y", {})
        )
            ...
            for element in root.iter():
                ...
                if tag == "path":
                    ...
                    if "bar" in aria_desc.lower():
                        ...
                        parsed = CrossPlatformMetadataExporter._parse_generic_path(
                            element
                        )
                elif tag == "circle":
                ...
                if parsed:
                    ...
                    for k, v in element.attrib.items():
                        if k.startswith("data-"):
                            key = k[5:]  # remove 'data-'
                            data[key] = CrossPlatformMetadataExporter._try_parse_number(
                                v
                            )
                        elif k == "aria-label":
                            data.update(
                                CrossPlatformMetadataExporter._parse_aria_label(v)
                            )
    ...
    def _parse_aria_label(aria_label: str) -> Dict[str, Any]:
        ...
        for part in parts:
            if ":" in part:
                ...
                data[key.strip()] = CrossPlatformMetadataExporter._try_parse_number(
                    value.strip()
                )
        return data
