...
def _binnify_labels(all_labels: Set[str], data_df: pd.DataFrame, highlight_name: Optional[str], bins: int = 8):
    ...
    try:
        ...
        data_df.loc[non_highlight_mask, 'label_numeric'] = pd.to_numeric(data_df.loc[non_highlight_mask, 'label'],
                                                                         errors='raise')
        ...
        raise ValueError(f"Found non-numeric label in continuous dataset in projection plotting!")
    return data_df
...
def plot_projection_result(projection_result: ProjectionResult,
    ...
    chart = alt.Chart(df_binned).mark_circle(size=60).encode(
