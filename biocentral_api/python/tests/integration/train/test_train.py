...
from biocentral_api import CommonEmbedder, SequenceData, Protocol
...
class TestTrainAndInference(unittest.TestCase):
    ...
    def test_train_then_infer(self):
        ...
        training_data = [
            ...
            SequenceData(
                seq_id="Seq2", seq="PRTEIN", label="Membrane", set="train", mask=None
            ),
            SequenceData(
                seq_id="Seq3", seq="PRT", label="Soluble", set="train", mask=None
            ),
            SequenceData(
                seq_id="Seq4", seq="SEQWENCE", label="Membrane", set="val", mask=None
            ),
            SequenceData(
                seq_id="Seq5", seq="PRTE", label="Soluble", set="val", mask=None
            ),
            SequenceData(
                seq_id="Seq6", seq="MMALSM", label="Membrane", set="test", mask=None
            ),
            SequenceData(
                seq_id="Seq7", seq="PRSEQ", label="Soluble", set="test", mask=None
            ),
        ]
        ...
        inference_result = self.api.inference(
