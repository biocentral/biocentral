    ...
    def update_tqdm(self, dtos: List[TaskDTO], pbar: tqdm) -> tqdm:
        for dto in dtos:
            ...
            if status == TaskStatus.FAILED:
                pbar.set_description("Training failed")
                pbar.close()
                break
        return pbar
        ...
        for dto in dtos:
            ...
            match status:
                case TaskStatus.PENDING:
                    pbar.set_description("Waiting for inference to start..")
                case TaskStatus.RUNNING:
                    pbar.set_description("Running inference..")
                case TaskStatus.FINISHED:
                    pbar.set_description("Finished inference!")
                    break
                case TaskStatus.FAILED:
                    pbar.set_description("Inference failed!")
                    break
        return pbar
...
class CustomModelsClient(ClientInterface):
    def train(
        ...
        start_training_request = StartTrainingRequest(
            config_dict=config, training_data=training_data
        )
        ...
        return BiocentralServerTask(
            task_id=task_id, api_client=api_client, dto_handler=training_dto_handler
        )
    ...
    def inference(
        ...
        start_inference_request = StartInferenceRequest(
            model_hash=model_hash, sequence_data=inference_data
        )
        task_id = self._submit_task(
        ...
        return BiocentralServerTask(
            task_id=task_id, api_client=api_client, dto_handler=_InferenceDTOHandler()
        )
