...
from .._generated import ApiClient, TaxonomyRequest, ProteinsApi, TaxonomyItem, TaxonomyResponse, ClusteringRequest, \
    TaskDTO, TaskStatus
        ...
        for dto in dtos:
            ...
            match status:
                case TaskStatus.PENDING:
                    pbar.set_description(f"Waiting for clustering task to start..")
                case TaskStatus.RUNNING:
                    pbar.set_description(f"Clustering..")
                case TaskStatus.FINISHED:
                    pbar.set_description(f"Finished clustering!")
                    break
                case TaskStatus.FAILED:
                    pbar.set_description(f"Clustering failed!")
                    break
        return pbar

    def get_tqdm_initial_description(self) -> str:
        return f"Running clustering.."
...
class ProteinsClient(ClientInterface):
    def taxonomy(self, api_client: ApiClient, taxonomy_ids: List[int]) -> Optional[List[TaxonomyItem]]:
        ...
        try:
            ...
            taxonomy_response: TaxonomyResponse = api_instance.taxonomy_api_v1_protein_service_taxonomy_post(
                taxonomy_request)
    ...
    def cluster(self, api_client: ApiClient, sequence_data: Dict[str, str], sequence_identity_threshold: float):
        ...
        api_instance = ProteinsApi(api_client)
        clustering_request = ClusteringRequest(sequence_data=sequence_data,
        ...
        biocentral_server_task = BiocentralServerTask(task_id=task_id,
                                                      ...
                                                      dto_handler=cluster_dto_handler)
