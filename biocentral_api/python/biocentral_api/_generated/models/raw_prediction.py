...
class RawPrediction(BaseModel):
    ...
    anyof_schema_3_validator: Optional[List[Any]] = None
    if TYPE_CHECKING:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        instance = RawPrediction.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in RawPrediction with anyOf schemas: List[object], float, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into RawPrediction with anyOf schemas: List[object], float, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], List[object], float, str]]:
