...
class McdStd(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in McdStd with anyOf schemas: List[float], float. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into McdStd with anyOf schemas: List[float], float. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
