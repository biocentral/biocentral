...
description: Learn how to train a model with biotrainer.
icon: Rocket
---
...
Thus, our problem belongs to the [residue_to_class protocol](data_standardization.mdx#residue_to_class).
...
to test other [embedding methods](config_file_options.mdx#embeddings) as well! Just keep in mind that some of them
